// src/components/admin/customers/admin-customer-create-modal.tsx
"use client";

import { useState } from "react";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { X, Eye, EyeOff, CheckCircle2, UserPlus } from "lucide-react";
...
export function AdminCustomerCreateModal({ onClose }: Props) {
  const queryClient = useQueryClient();

  const [fullName, setFullName] = useState("");
  const [email, setEmail] = useState("");
  const [role, setRole] = useState<"owner" | "super_admin" | "reseller">("owner");
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const body = { fullName, email, password, role, isActive };
      const res = await apiClient.post("/admin/customers", body);
      return res.data;
    },
    onSuccess: () => {
      setSuccess(true);
  ...
  function handleSubmit() {
    ...
    if (!/[A-Z]/.test(password)) { setFieldError("Password must include at least one uppercase letter."); return; }
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "var(--border)" }}>
          <div className="flex items-center gap-2 text-[var(--brand-500)]">
            ...
            <span className="text-sm font-semibold text-[var(--foreground)]">Insert User</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3 py-1">
            ...
            <span className="text-xs text-[var(--subtle-text)] font-medium">Password Settings</span>
            <div className="flex-1 h-px bg-[var(--border)]" />
          </div>
